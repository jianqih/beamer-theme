...
\usepackage{tikz}
\usepackage{verbatim}
\setbeamertemplate{note page}{\pagecolor{pink!5}\insertnote}
...
\usepackage{mathpazo} % Loads Palatino text and matching math fonts
\usefonttheme{professionalfonts}  % Prevents Beamer from overriding math fonts
\renewcommand{\familydefault}{\rmdefault} % Forces Beamer text to use the Serif (Palatino) font
...
\definecolor{blue}{HTML}{4A8DDA}
% {RGB}{0,120,140}
% 2B5F7C
% {HTML}{2F90B9} % 云山蓝 
\definecolor{red}{HTML}{D55E00}
\definecolor{pink}{HTML}{EA7293}
\definecolor{green}{HTML}{3B5F3A}
...
\definecolor{MyBackground}{HTML}{FFFDD9}
...
\title[]{{\scshape\Huge\textbf{\textcolor{blue}{Economics of AI}}} \\ \large{How to make good slides with \LaTeX}}
\author[PGP]{}
\institute{\large{\textbf{Jianqi Huang}} \\\small HKUST(Guangzhou)}
...
\begin{frame}{Math}
    Use the \texttt{equation} environment to make your math look nice and centered. For example, the utility function of a representative agent is given by:
    \begin{equation}
        U_t = \frac{c^{1-\sigma }}{1-\sigma}
    \end{equation}
    where $\sigma $ is the CRRA coefficient and $c$ is consumption.
\end{frame}
...
\begin{frame}[fragile]{I use two to three colors regularly throughout my presentations}
  \begin{wideitemize}
    ...
    \item[-] I use \textcolor{pink}{pink}  as a transition color
  \end{wideitemize}
\medskip
...
\begin{frame}{Add referfences and bibliography}
  \begin{wideitemize}
    \item Use the \texttt{\textbackslash bibliography\{refs\}} command to add your bibliography
    \item Use the \texttt{\textbackslash bibliographystyle\{econ-econometrica\}} command to set the style
    \item You can use any .bib file, but I like to keep mine in the same folder as my slides
  \end{wideitemize}
  \vspace*{1em}

  For example, \citet{acemoglu2024} study the impact of AI on labor markets and find that while AI has the potential to increase productivity, it also poses significant challenges for workers, particularly those in routine jobs. They suggest that policy interventions may be necessary to mitigate the negative effects of AI on employment and income inequality.
\end{frame}
